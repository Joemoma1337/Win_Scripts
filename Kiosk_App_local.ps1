...
.DESCRIPTION
    ...
    1.  Sets registry keys to enable automatic logon for the 'kioskuser'.
    2.  Sets the 'Shell' registry key to launch the specified executable instead of the
...
# The full path to the executable you want to run in kiosk mode.
# This must be a standalone .exe file.
...
Write-Host "Changing the system shell to '$exePath'..." -ForegroundColor Yellow
...
# Set the new shell to the specified program
try {
    Set-ItemProperty -Path $winlogonPath -Name Shell -Value $exePath -Force
    Write-Host "Shell successfully set to '$exePath'." -ForegroundColor Green
}
catch {
